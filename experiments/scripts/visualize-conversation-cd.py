...
def visualize_discussion_tree(conv: Conversation, ignore_shallow: bool = True):
    ...
    sampled_colors = r.sample(COLORS, len(authors_index)) if len(authors_index) < len(COLORS) else COLORS
    author_colors = {a: sampled_colors[i % len(COLORS)] for a, i in authors_index.items()}
