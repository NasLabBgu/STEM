...
if __name__ == "__main__":
    ...
    parser.add_argument("changli_dataset", type=str,
                        help="path to the directory where the dataset used by the paper is stored")
    parser.add_argument("iac_4forums_dataset", type=str,
                        help="path to the directory where the dataset used by the paper is stored")
    parser.add_argument("--outpath", "-o", type=str, default=None,
                        help="file path where to output the prepared data for experiments")
    args = parser.parse_args()

    print(f"Reading data from {args.changli_dataset}")
    authors_path = os.path.join(args.changli_dataset, "allPostAuthorMap.pickle")
    labels_path = os.path.join(args.changli_dataset, "allPostLabelMap.pickle")
    threads_path = os.path.join(args.changli_dataset, "allThreadPost.pickle")
    replies_path = os.path.join(args.changli_dataset, "allPostLinkMap.pickle")

    print(f"Reading data from {args.iac_4forums_dataset}")
    authors_index_path = os.path.join(args.iac_4forums_dataset, "author.txt")
    topic_index_path = os.path.join(args.iac_4forums_dataset, "topic.txt")
    original_posts_path = os.path.join(args.iac_4forums_dataset, "post.txt")
    ...
    outpath = args.outpath or os.path.join(args.changli_dataset, "4forums-changli-records.csv")
